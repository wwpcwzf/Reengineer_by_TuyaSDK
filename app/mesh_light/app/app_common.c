...
{
    tuya_mesh_data_send(get_primary_ele_addr(), DstAddr, VD_TUYA_WRITE_NOACK, VendorData, DataLen, 0, 0);
}
...
static int app_light_group_addr_inquire_callback(void){
    ...
    tuya_mesh_data_send(get_primary_ele_addr(), inquire_src, VD_TUYA_WRITE_NOACK, buf, 4, 0, 0);
...
void tuya_mesh_data_recv_callback(uint16_t src_addr, uint16_t dst_addr, uint32_t opcode, uint8_t *data, uint16_t data_len, uint8_t ttl)
{
    ...
    switch(opcode){
        ...
        case TUYA_LIGHT_CTL_GET:{
            ...
            u16 tuya_temp = app_light_ctrl_data_bright_get(); 
        ...
        case TUYA_VD_TUYA_WRITE_NOACK:
            {
                if((((data[0]==1)&&(data[1]==6))||((data[0]==2)&&(data_len<2))) && ((dst_addr & 0xC000) == 0xC000))//鍦烘櫙鍛戒护鎴栬�呭彂鍦烘櫙妯″紡 鎷垮埌鍦板潃
                {
